...
class BulkUnenrollTests(SharedModuleStoreTestCase):
    ...
    def test_users_unenroll_successfully_logged(self):
        ...
        for enrollment in self.enrollments:
            ...
            lines += username + "," + str(enrollment.course.id) + "\n"
        ...
        with LogCapture(LOGGER_NAME) as log:
            call_command("bulk_unenroll")
            for key, value in users_unenrolled.items():
                username, course = key, value[0].decode('utf-8')
            log.check(
                (
                    LOGGER_NAME,
                    'INFO',
                    'User [{}] have been successfully unenrolled from the course: {}'.format(username, course)
                    )
                )
